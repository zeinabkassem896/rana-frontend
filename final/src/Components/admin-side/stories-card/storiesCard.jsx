...
const StoriesAdmin = ({ story, handleChangeObject, refresh }) => {
  const deleteStory = async (id) => {
    try {
      const response = await axios.delete(
        import.meta.env.VITE_API_ENDPOINT + `api/stories/${story._id}`
      );
  ...
  return (
    <div className="stories-admin-container">
      <div className="card" key={story._id}>
        <div className="stories-img-container">
        <img src={import.meta.env.VITE_API_ENDPOINT+story.image} alt="" />
        </div>
       
        <div className="card-content-stories">
          <p className="card-title-stories">{story.title}</p>
          <p className="card-text-stories">{story.text}</p>
          <div className="button-container-stories">
          <span className="edit-span">
            <button
              className="edit-button"
              onClick={() => {
                handleChangeObject(story);
              }}
            >
              <svg
                className="svg-icon"
